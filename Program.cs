...
namespace Lab1
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            double y = -0.0049;
            double A = Math.Sqrt(Math.Log10(4 / 3 + x)) - Math.Pow(Math.E, - Math.Sin(1.3 * x - 0.7));// math expression
            Console.WriteLine($" x = {x}, y = {y}, A = {A}");// output result
            double x1 = 2.5;// second value
            double A1 = Math.Sqrt(Math.Log10(4 / 3 + x1)) - Math.Pow(Math.E, - Math.Sin(1.3 * x1 - 0.7)); //math expression
            Console.WriteLine($" x1 = {x1}, y = {y}, A1 = {A1}"); //output result
